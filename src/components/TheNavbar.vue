...
<template>
  <nav container px-6 py-1 mx-auto items-center md:flex md:justify-between md:items-center text-xl>
    ...
    <div :class="{ 'hidden': !menuOpen.valueOf, 'md:flex': false }" flex gap-4 mt-4 items-center md:mt-0>
        <!-- Menu Items -->
        <RouterLink v-for="item in menuItems" :key="item.route" :to="item.route" relative hover:text-pink-600 group>
          {{ t(item.label) }}
        ...
        <RouterLink flex gap-1 to="/" :title="t('button.home')">
          <div i-carbon-logo-instagram text-2xl hover:text-pink-600/>
        </RouterLink>

        <RouterLink flex gap-1 to="/" :title="t('button.home')">
          <div i-carbon-logo-discord text-2xl hover:text-pink-600 />
        </RouterLink>  

        <RouterLink flex gap-1 to="/" :title="t('button.home')">
          <div i-carbon-logo-twitter text-2xl hover:text-pink-600 />
        </RouterLink>  
